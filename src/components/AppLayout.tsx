...
const AppLayout: React.FC = () => {
  ...
  const shells = tokens
    ...
    .slice(0, 10);
  const featuredFood = storeProducts
      ...
      <main className="flex-1">
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-lg text-gray-400 mb-8 max-w-lg">
                  ...
                  rewards. The world's first food-backed trading platform with
                  60 tokens and 24hr perishable shells.
                </p>
                        ...
                        {t ? (
                          <>
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${t.token_type === "perishable_shell" ? "bg-orange-500/20 text-orange-400" : "bg-emerald-500/20 text-emerald-400"}`}
                              >
                                {t.symbol.slice(0, 2)}
                              </div>
                              <div>
        ...
        <section className="max-w-7xl mx-auto px-4 -mt-4 mb-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Total Tokens", value: "60", icon: BarChart3 },
              { label: "Commodity Coins", value: "30", icon: Carrot },
              { label: "Perishable Shells", value: "30", icon: Clock },
              { label: "Store Items", value: "100+", icon: Store },
            ].map((s, i) => (
              <div
                key={i}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {topGainers.map((t) => (
              <Link
                to="/trade"
                key={t.symbol}
                className="bg-[#1a1f2e] rounded-xl border border-gray-800 p-4 hover:border-emerald-500/30 transition-all"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold ${t.token_type === "perishable_shell" ? "bg-orange-500/20 text-orange-400" : "bg-emerald-500/20 text-emerald-400"}`}
                  >
                    {t.symbol.slice(0, 3)}
                  </div>
                  <div>
        ...
        <section className="max-w-7xl mx-auto px-4 mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              <Store className="w-5 h-5 text-emerald-400" /> Featured Dishes
            </h2>
            <Link
              to="/store"
              className="text-sm text-emerald-400 hover:text-emerald-300 flex items-center gap-1"
            ...
            {featuredFood.map((product) => (
              <div
                ...
                onClick={() => {
                  ...
                  setSelectedSpice(product.spiceLevels[0]);
                  setQty(1);
                }}
        ...
        <section className="max-w-7xl mx-auto px-4 mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className="bg-[#1a1f2e] rounded-xl border border-gray-800 p-6 hover:border-gray-700 transition-all"
              >
            ...
            <div className="p-6 -mt-6 relative">
                ...
                <div className="flex gap-2">
                  {selectedProduct.spiceLevels.map((s) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${selectedSpice === s ? "bg-orange-500/20 text-orange-400 border border-orange-500/30" : "bg-white/5 text-gray-400 border border-gray-700"}`}
                    >
                      {s}
                    </button>
                  ))}
              ...
              <div className="mb-4 flex items-center gap-3">
                <button
                  ...
                  className="p-2 bg-white/5 border border-gray-700 rounded-lg text-white"
                >
                    ...
                    {similarProducts.map((p) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setSelectedSpice(p.spiceLevels[0]);
                          setQty(1);
                        }}
                        className="bg-white/5 rounded-lg overflow-hidden cursor-pointer hover:bg-white/10"
                      >
                        <img
                          src={p.image}
                          alt={p.name}
